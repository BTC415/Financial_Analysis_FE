...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col items-center w-[88px]  bg-[#2E5E9E] h-screen">
      <div className="w-[20rem] h-[16rem] flex justify-center items-center border-b-2 border-b-gray-400">
        <svg width={40} height={40}><use href="#svg-logo" /></svg>
      </div>
      <div className="mt-8 w-[20rem] ">
        <button className={`w-[20rem] h-[16rem] flex items-center justify-center ${clicked === "servico" ? 'border-lime-400 border-l-4' : ''}`} >
          <svg width={24} height={24} className="mx-auto items-center"><use href={`${clicked === "servico" ? '#svg-servico-yellow' : '#svg-servico'} `} /></svg>
        </button>
        <button className={`w-[20rem] h-[16rem] flex items-center justify-center ${clicked === "historico" ? 'border-lime-400 border-l-4' : ''}`}>
          <svg width={24} height={24} className="mx-auto "><use href={`${clicked === "historico" ? '#svg-historico-yellow' : '#svg-historico'} `} /></svg>
        </button>
        <button className={`w-[20rem] h-[16rem] flex items-center justify-center ${clicked === "wallet" ? 'border-lime-400 border-l-4' : ''}`} onClick={() => setClicked('wallet')}>
